...
function voteadapter(kill, message, pollfile, voterslistfile) { // fique atento ao que está apos o function, isso ja ira te definir o que é
    ...
    if (data['candis'] === 'null') {
        kill.reply(message.chatId, '⭐️ Bueno, no tenemos otra opción ... ¿Podría el creador de la encuesta olvidarse de publicar?', message.id, true);
        return;
    }
    let arr = data['candis']
...
async function addcandidate(kill, message, candi, pollfile, voterslistfile) {
    ...
    if (data['candis'] === 'null') {
        ...
        if (data['candis'].length >= 10) {
            kill.reply(message.chatId, '🎯️El número máximo de candidatos que puedo agregar es 10, lo siento.', message.id, true);
            return;
        }
        let cd = {
            name: candi,
    ...
    kill.reply(message.chatId, `🎯️ ¡OK! Se agregó el candidato $ {candi}!`, message.id, true);
}
